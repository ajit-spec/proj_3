...
export class LoginComponent implements OnInit {
  ...
  form = this.formbuilder.group(
    {
      email: [
        '',
        Validators.compose(
          [Validators.required, this.service3.checkvalidemail]
        )
      ],
      password: [
        '',
  ...
  ngOnInit(): void {
    if (this.service4.isauthenticated() && !this.service4.getcurrentuser().isadmin) {
      this.router.navigate(['/', 'product'])
    } else if (this.service4.isauthenticated() && this.service4.getcurrentuser().isadmin) {
    ...
    this.service1.login(request).subscribe(value => {
      ...
      if (value.status === 1) {
        ...
        setTimeout(() => {
          ...
          if (this.service4.getcurrentuser().isadmin) {
            this.router.navigate(['/', 'admin'])
          } else {
            this.router.navigate(['/', 'product'])
          }
